...
/**
 * parseNumberSmart
 * - Si viene "28.49" → 28.49 (punto = decimal)
 * - Si viene "28,49" → 28.49 (coma = decimal)
 * - Si viene "1.234,56" → 1234.56 (puntos como miles + coma decimal)
 * - Si viene "1,234.56" → 1234.56 (comas como miles + punto decimal)
 */
function parseNumberSmart(v) {
  ...
  // Solo dígitos y opcional decimal con punto (28.49)
  if (/^-?\d+(\.\d+)?$/.test(s)) return Number(s);

  // Solo dígitos y opcional decimal con coma (28,49)
  if (/^-?\d+(,\d+)?$/.test(s)) return Number(s.replace(',', '.'));

  // Mezcla de miles y decimales. Normalizamos:
  // 1) Quitamos separadores de miles según patrón (punto o coma antes de 3 dígitos)
  // 2) Convertimos el separador decimal a punto.
  // Ej: "1.234,56" -> "1234,56" -> "1234.56"
  //     "1,234.56" -> "1234.56"  (quita coma miles, deja punto decimal)
  const noSpaces = s.replace(/\s/g, '');

  // Quitar puntos que actúan como miles (p.ej. 1.234, 12.345)
  const dropThousandDots = noSpaces.replace(/\.(?=\d{3}(?:\D|$))/g, '');
  // Quitar comas que actúan como miles (p.ej. 1,234)
  const dropThousandCommas = dropThousandDots.replace(/,(?=\d{3}(?:\D|$))/g, '');

  // Si quedó una coma, asumir que es decimal
  const normalized = dropThousandCommas.includes(',')
    ? dropThousandCommas.replace(',', '.')
    : dropThousandCommas;
...
function prepareWide(rows, { dateField, parsers, columns }) {
  return rows.map(r => {
    ...
    columns.forEach(c => { out[c.key] = parseNumberSmart(r[c.key]); });
    return out;
  });
}
...
function prepareLong(rows, { dateField, categoryField, valueField, parsers }) {
  const normalized = rows.map(r => ({
    ...
    value: parseNumberSmart(r[valueField]),
