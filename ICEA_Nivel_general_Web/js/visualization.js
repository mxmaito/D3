...
import { renderAxes }     from "./axes-and-grid.js";         // (g, x, y, innerW, innerH)
import { drawGridlines }  from "./axes-and-grid.js";    // (g, x, y, innerW, innerH)
import {
...
import { addTooltip }    from './tooltip.js';       // (g, overlay, data, columns, x, y, state, bus)
...
export function visualizeData({
  ...
  // IMPORTANT: d3.dispatch tipos SIN puntos. Los puntos son para namespaces en `.on`.
  const bus = d3.dispatch('hover', 'leave', 'toggle', 'labelsToggle', 'tooltipToggle');
  ...
  bus.on('toggle.viz', (key) => {
    ...
    bus.call('labelsToggle', null, key);
    bus.call('tooltipToggle', null, key);
  });
  ...
  ({ labelsMap } = addRightLabels(uiLayer, data, columns, x, y, innerWidth, state, bus));
  applyVisibility(seriesMap, state);

  const tooltip = addTooltip(g, overlay, data, columns, x, y, state, bus);
