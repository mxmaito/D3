...
export function addTooltip(rootG, overlay, data, columns, x, y) {
  ...
  const guideline = layer.append('line').attr('y1', 0).attr('y2', +rootG.attr('data-inner-height')).attr('stroke', '#111827').attr('stroke-dasharray', '3,3').attr('opacity', 0.6);
  const dots = layer.selectAll('circle.dot').data(columns.map(c => c.key)).join('circle').attr('class', 'dot').attr('r', 3.5).attr('fill', k => COLOR(k)).attr('stroke', '#fff').attr('stroke-width', 1.25).attr('opacity', 0);
  const box = d3.select('body').append('div').attr('class', 'd3-tooltip').style('position', 'fixed').style('background', 'rgba(17,24,39,.95)').style('color', '#fff').style('padding', '8px 10px').style('border-radius', '8px').style('font', '12px/1.2 system-ui, sans-serif').style('pointer-events', 'none').style('opacity', 0);
  ...
  const onMove = throttle(function (event) {
    ...
    dots.each(function (key) {
      ...
      d3.select(this).attr('cx', x(d.date)).attr('cy', y(v)).attr('opacity', show ? 1 : 0);
    });
    box.html(`
  ...
  const onLeave = () => { box.style('opacity', 0); dots.attr('opacity', 0); };
